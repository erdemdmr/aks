...
create() {
    ...
    create_role_assignment "${aksName}"
    ...
    az group wait --exists  --resource-group "${aksResName}"

    set_resource_group_tags "${aksResName}"
    create_role_assignment "${aksResName}"

    if [ -n "${aadGroupId}" ]; then
        apply_clusterrolebinding "${aadGroupId}"
    fi

    info "AKS cluster will be create: ${aksName}"
}
...
set_tags() {
    ...
    #tags="Creator=${currentUser}, User=${email}"
    tags="User=${email}"
}
...
create_role_assignment() {
    info "Creating role assigments ..."
 
    az group wait --exists  --resource-group "${1}"
    az role assignment create --role "Contributor" --assignee ${email} --resource-group ${1} >/dev/null ||
        fatal "Could not create role assigments!"
}
